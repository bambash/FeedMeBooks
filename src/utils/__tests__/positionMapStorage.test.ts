import AsyncStorage from '@react-native-async-storage/async-storage';
import { savePositionMap, loadPositionMap, deletePositionMap } from '../positionMapStorage';
import type { AudiobookPositionMap } from '../../types';

const mockStore = new Map<string, string>();

jest.mock('@react-native-async-storage/async-storage', () => ({
  setItem: jest.fn(async (key: string, value: string) => {
    mockStore.set(key, value);
  }),
  getItem: jest.fn(async (key: string) => mockStore.get(key) ?? null),
  removeItem: jest.fn(async (key: string) => {
    mockStore.delete(key);
  }),
}));

const MOCK_MAP: AudiobookPositionMap = {
  bookId: 'book-abc',
  createdAt: 1_700_000_000_000,
  totalAudioMs: 3_600_000,
  chapterAnchors: [
    { chapterIndex: 0, withinChapterFraction: 0, audioMs: 0, source: 'forced-alignment', confidence: 0.94 },
    { chapterIndex: 1, withinChapterFraction: 0, audioMs: 30_000, source: 'proportional-fallback' },
    { chapterIndex: 2, withinChapterFraction: 0.5, audioMs: 60_000, source: 'confirmed' },
  ],
  builtFrom: 'transcript',
};

const STORAGE_KEY = 'feedmebooks:positionmap:book-abc';

beforeEach(() => jest.clearAllMocks());
beforeEach(() => mockStore.clear());

describe('positionMapStorage', () => {
  it('round-trips a mixed-source map', async () => {
    await savePositionMap(MOCK_MAP);
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(STORAGE_KEY, JSON.stringify(MOCK_MAP));
    const result = await loadPositionMap('book-abc');
    expect(result).toEqual(MOCK_MAP);
    expect(AsyncStorage.getItem).toHaveBeenCalledWith(STORAGE_KEY);
  });

  it('returns null when key does not exist', async () => {
    mockStore.clear();
    expect(await loadPositionMap('book-abc')).toBeNull();
  });

  it('returns null when stored value is malformed JSON', async () => {
    mockStore.set(STORAGE_KEY, '{invalid json');
    expect(await loadPositionMap('book-abc')).toBeNull();
  });

  it('removes the correct key', async () => {
    await deletePositionMap('book-abc');
    expect(AsyncStorage.removeItem).toHaveBeenCalledWith(STORAGE_KEY);
  });
});
